...
import type { HealthContext } from "@fit-analyzer/shared";
...
async function resolveActiveHealthContext(
	...
	const tryHae = async (): Promise<HealthContext | null> => {
		try {
			...
			console.warn("[health_data] HAE health context fetch failed:", err);
			return null;
		}
	};
	const tryOw = async () => {
		try {
			...
			console.warn("[health_data] OW health context fetch failed:", err);
			return null;
		}
	};
...
export const healthDataDefinition: ToolDefinition = {
	name: "health_data",
	description:
		"Retrieve the athlete's health and fitness context including wearable health metrics (resting HR, HRV, sleep, SpO2), body composition, user-provided profile (FTP, max HR, goals), and recent training history. Call this when you need athlete context to personalize coaching advice.",
	parameters: {
		type: "object",
		properties: {},
		required: [],
...
export const healthDataHandler: ToolHandler = async (_args, context) => {
	const userId = context.userId;

	try {
		const { context: healthCtx, sourceLabel } =
			await resolveActiveHealthContext(userId);
		...
		if (profileText) {
			...
			sections.push(
				"User-provided settings that override estimated values. Prefer these over estimates when they exist.",
			);
			sections.push(profileText);
		}

		if (healthCtx) {
			const healthText = formatHealthContext(healthCtx, sourceLabel);
			if (healthText) {
				sections.push("## Athlete Health Data");
				sections.push(
					"Use this data to contextualize training advice (fatigue, recovery, sleep). Fetched live from the athlete's wearables.",
				);
				sections.push(healthText);
			}
		}

		// Body composition is OW-only today
		if (getOwUserId(userId)) {
			try {
				...
				if (bodyText) {
					const source = body?.source.device
						? `${body.source.provider} (${body.source.device})`
						: (body?.source.provider ?? "OpenWearables");
					sections.push("## Athlete Body Composition");
					sections.push(
				...
				console.warn("[health_data] OW body summary fetch failed:", err);
			}
		}

		if (activityStats.count > 0) {
			sections.push("## Recent Training History");
			sections.push(
			...
			if (estimatedVo2max != null)
				estimates.push(`- Estimated VO₂max: ${estimatedVo2max} ml/kg/min`);
			if (estimates.length > 0) {
				sections.push("## Fitness Estimates");
				sections.push("Computed from the athlete's full training history.");
				sections.push(estimates.join("\n"));
			}
		}
		...
		if (!content.trim()) {
			return {
				id: "",
				name: "health_data",
				content: "No health or fitness data available for this athlete yet.",
				display: null,
			};
		}
