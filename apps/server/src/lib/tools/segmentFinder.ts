...
export const segmentFinderHandler: ToolHandler = async (args, context) => {
	...
	const findDescents = segmentTypeRaw === "all" || segmentTypeRaw === "descent";
	...
	for (const seg of merged) {
		...
		const avgGradient = gradientCount > 0 ? gradientSum / gradientCount : null;
	...
	for (const seg of results) {
		...
		if (seg.avgGradient != null) parts.push(`${seg.avgGradient}% avg gradient`);
