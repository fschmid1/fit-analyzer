import type { StoredRecord, WaxedChainReminderSettings } from "@fit-analyzer/shared";
...
function sanitizeThresholdKm(value: number): number {
  return Number.isFinite(value) && value > 0 ? Math.round(value) : DEFAULT_THRESHOLD_KM;
}
...
function toPublicSettings(row: UserSettingsRow | null): WaxedChainReminderSettings {
  const thresholdKm = sanitizeThresholdKm(row?.waxed_chain_reminder_km ?? DEFAULT_THRESHOLD_KM);
  const accumulatedKm = Math.max(0, row?.waxed_chain_accumulated_km ?? 0);
  const remainingKm = accumulatedKm >= thresholdKm ? 0 : thresholdKm - accumulatedKm;
...
async function sendWaxedChainReminder(topic: string, thresholdKm: number, accumulatedKm: number) {
  if (!env.NTFY_HOST) {
...
export async function handleNewActivityForWaxedChainReminder(
  ...
  const nextAccumulatedKm = (row?.waxed_chain_accumulated_km ?? 0) + activityDistanceKm;

  if (nextAccumulatedKm < settings.thresholdKm) {
    updateAccumulatedStmt.run(userId, nextAccumulatedKm, row?.waxed_chain_last_notified_at ?? null);
    return;
  }

  try {
    await sendWaxedChainReminder(settings.ntfyTopic, settings.thresholdKm, nextAccumulatedKm);
    ...
    updateAccumulatedStmt.run(userId, nextAccumulatedKm, row?.waxed_chain_last_notified_at ?? null);
    console.error(`[waxed-chain] Failed to send reminder for user ${userId}:`, error);
  }
}
