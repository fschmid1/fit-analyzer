...
type LegacyActivitySummary = Omit<ActivitySummary, "totalDistanceKm">;
type PersistedActivitySummary = ActivitySummary | LegacyActivitySummary;

function hydrateSummaryDistance(
	summary: PersistedActivitySummary,
	...
	const computedDistanceKm =
		records && records.length > 1
			? Math.round(computeDistanceKm(records) * 10) / 10
			: null;
	...
	return {
		summary: {
			...summary,
			totalDistanceKm:
				computedDistanceKm && computedDistanceKm > 0
					? computedDistanceKm
					: null,
		},
		repaired: true,
