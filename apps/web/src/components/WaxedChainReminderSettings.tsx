...
export function WaxedChainReminderSettings() {
  const [settings, setSettings] = useState<WaxedChainReminderSettings | null>(null);
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="p-5 bg-[#1a1533]/70 border border-[rgba(139,92,246,0.15)] rounded-xl flex flex-col gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-[#f1f5f9]">Waxed chain reminders</p>
            <p className="text-xs text-[#94a3b8]">
              Send an `ntfy` notification when newly added rides cross your maintenance threshold.
            </p>
          </div>
          {loading && <Loader2 className="w-4 h-4 text-[#8b5cf6] animate-spin ml-auto" />}
        </div>
        ...
        {!loading && (
          <>
            <label className="flex items-center justify-between gap-4 rounded-xl border border-[rgba(139,92,246,0.12)] bg-[#0f0b1a]/70 px-4 py-3">
              <div>
                <p className="text-sm font-medium text-[#f1f5f9]">Enable reminders</p>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="flex flex-col gap-1.5">
                <span className="text-xs font-medium text-[#cbd5e1]">Reminder after km</span>
                <input
                  type="number"
                  min="1"
                  step="1"
              ...
              <label className="flex flex-col gap-1.5">
                <span className="text-xs font-medium text-[#cbd5e1]">`ntfy` topic</span>
                <input
                  type="text"
                  value={ntfyTopic}
            ...
            {settings && (
              <div className="rounded-xl border border-[rgba(139,92,246,0.12)] bg-[#0f0b1a]/70 px-4 py-3 text-xs text-[#94a3b8] flex flex-col gap-1">
                <p>
                  {settings.accumulatedKm.toFixed(1)} km tracked toward the next reminder.{" "}
                  {settings.remainingKm.toFixed(1)} km remaining.
                </p>
