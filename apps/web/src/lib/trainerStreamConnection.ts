...
export function createTrainerStreamConnection(threadId: string): TrainerStreamConnection {
  const queue = createQueue();
  let resumeStarted = false;

  const runStream = async (request: Promise<Response>, activeThreadId: string) => {
    let shouldClearActiveStream = false;
    try {
      const response = await request;
      const completionState = await streamSseResponse(response, (chunk) => queue.push(chunk));
