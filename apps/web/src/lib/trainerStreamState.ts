...
function readActiveStreams(): Record<string, ActiveTrainerStream> {
  return readPersistentJson<Record<string, ActiveTrainerStream>>(ACTIVE_STREAMS_KEY) ?? {};
}

function readDrafts(): Record<string, SerializableUIMessage[]> {
  return readJson<Record<string, SerializableUIMessage[]>>(DRAFT_MESSAGES_KEY) ?? {};
}
...
export function loadActiveTrainerStream(threadId: string): ActiveTrainerStream | null {
